...
public class StorageActionReceiver extends BroadcastReceiver {
    ...
    private void onMemcardMounted(Context context) {
    	// do not interrupt current operation, so no need to reload all.
    	/*
		...
		// reload everything
		CharSequence text = context.getString(R.string.app_name).trim() + ": " + context.getString(R.string.mounting_external_storage);
		if (MFPAdapter.canReloadAll())	{
