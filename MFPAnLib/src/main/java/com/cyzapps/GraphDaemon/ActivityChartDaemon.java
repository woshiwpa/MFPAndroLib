...
public class ActivityChartDaemon extends Activity {
    ...
    public void showErrorMsg(String strMsg, final boolean bExitActivity)	{
		...
		alertDialog = new AlertDialog.Builder(ActivityChartDaemon.this)
    ...
    public void startCfgOGLChart()	{
    	if (((OGLChart)mmfpChart).mbCfgWindowStarted == false)	{
    		...
    		intentConfigXYZGraph.putExtra("NotShowAxis", ((OGLChart)mmfpChart).getNotDrawAxis());
    		intentConfigXYZGraph.putExtra("NotShowTitle", ((OGLChart)mmfpChart).getNotDrawTitle());
    ...
    public void finish()	{
    	...
    	if (mlPlottingTime != 0)	{
	    	...
	    	if (bmp != null)	{
				...
				File folder = new File(strRootPath + LangFileManager.STRING_PATH_DIVISOR
										+ MFP4AndroidFileMan.STRING_CHART_SNAPSHOT_FOLDER
										+ LangFileManager.STRING_PATH_DIVISOR);
	    		...
	    		try {
					...
					// a lot of exceptions were thrown in google play.
					e1.printStackTrace();
				}
	    	}
    	}
