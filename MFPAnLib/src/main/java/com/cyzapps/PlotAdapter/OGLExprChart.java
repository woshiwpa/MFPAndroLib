...
import com.cyzapps.GraphDaemon.ActivityConfig3DExprGraph.AdjOGLExprChartParams;
...
public class OGLExprChart extends OGLChart {
	...
	public AdjOGLExprChartParams getChartDataParams()	{
        ...
        adjParam.mbNotShowAxis = mbNotDrawAxis;
        adjParam.mbNotShowTitle = mbNotDrawTitle;
		return adjParam;
	}
	...
	public synchronized void applyCfgChart(AdjOGLExprChartParams adjParams)	{
		...
		for (int idx = 0; idx < m3DExprSurfaces.length; idx ++)	{
   		...
   		if (!adjParams.isNoAdj(dXMin, dXMax, nXNumOfSteps, dYMin, dYMax, nYNumOfSteps, dZMin, dZMax, nZNumOfSteps, mbNotDrawAxis, mbNotDrawTitle) && mdlgDataRecalcing == null)	{
            ...
            mbNotDrawAxis = adjParams.mbNotShowAxis;
            mbNotDrawTitle = adjParams.mbNotShowTitle;
	...
	public void clickCfgBtn()	{
		((ActivityChartDaemon) mcontext).startCfgOGLExprChart();
	}
	...
	public void draw(GL10 gl)	{
		if (mdXFrom != mcaXAxis.mdValueFrom || mdXTo != mcaXAxis.mdValueTo
			...
			if (CitingSpaceDefinition.noMFPSUserLibLoaded())	{	// before recalc, we may need reload libs.
				// set initial working dir inside readSettings.
				MFP4AndroidFileMan mfp4AndroidFileMan = new MFP4AndroidFileMan(mcontext.getAssets());
				// have to call readsettings again here because when reload, readsettings in main panel is not executed.
				// also, this statement has to be excuted before reload all lib because reloading libs needs to know which
				// sd card stores lib files.
				readAppSettings(mfp4AndroidFileMan);
				// only 3DExpr and 2DExpr charts need to reload the functions
				MFP4AndroidFileMan.reloadAllUsrLibs(mcontext, 2, null);	// use sync mode here
			}
				// update dataset.
