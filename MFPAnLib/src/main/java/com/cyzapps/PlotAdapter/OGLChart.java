...
import com.cyzapps.GraphDaemon.ActivityConfigXYZGraph.AdjOGLChartParams;
	...
	public void applyCfgChart(AdjOGLChartParams adjParams)	{
		if (!adjParams.isNoAdj())	{
	        ...
	        updateMapperTO2TO(0, 0, 0,
	                ...
	                1, 1, 1);		
	        mbNotDrawAxis = adjParams.mbShowAxisChange?(!mbNotDrawAxis):mbNotDrawAxis;
	        mbNotDrawTitle = adjParams.mbShowTitleChange?(!mbNotDrawTitle):mbNotDrawTitle;
	        update();
		}
	}
	
	public void clickCfgBtn()	{
		((ActivityChartDaemon) mcontext).startCfgOGLChart();
	}
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        if (mbTakeSnapshot == true)	{
			if (android.os.Build.VERSION.SDK_INT <= android.os.Build.VERSION_CODES.GINGERBREAD_MR1
                    && mbitmapScreenSnapshot != null && !mbitmapScreenSnapshot.isRecycled()) {
				mbitmapScreenSnapshot.recycle();	// release memory first for Android 2.3.3 or lower
			}
...
Log.d("OGLChart", "onSurfaceChanged : start, chart title is " + this.mstrChartTitle);
        ...
        setMapperFROM2TO();
    	// if it is 1st surface change, and x:y:z is not 1:1:1, we set it to be 1:1:1
        // otherwise, just update.
        if (mb1stSurfaceChange) {
        	if (mdXAxisLenInFROM != mdYAxisLenInFROM || mdXAxisLenInFROM != mdZAxisLenInFROM) {
        		adjustXYZ1To1();
        	} else {
        		update();
        	}
        	mb1stSurfaceChange = false;
        } else {
        	update();
        }
...
//Log.d("OGLChart", "draw : after draw surfaces");
        boolean bInLandScapeMode = (mdChartWindowWidth >= mdChartWindowHeight * 0.9);
		com.cyzapps.VisualMFP.Color colorChartTitle = mcolorForeGrnd;
		double dTitleSize = bInLandScapeMode?(mdSmallSize):(0.67*mdSmallSize);
...
//Log.d("OGLChart", "draw : after draw title");
        ...
        if (bInLandScapeMode)   {  // landscape mode
...
//Log.d("OGLChart", "draw : after draw buttons");
	...
	public AllCoordAxis updateCoordAxis(AxisMarks axisMarks)	{
		...
		MFPNumeric mfpNumIntTmp = new MFPNumeric((axisMarks.mdXMark1 - caXAxis.mdValueFrom)/caXAxis.mdMarkInterval, true).toIntOrNanInfMFPNum();
		double dXMarkStart = axisMarks.mdXMark1 - mfpNumIntTmp.multiply(new MFPNumeric(caXAxis.mdMarkInterval, true)).doubleValue();
		...
		mfpNumIntTmp = new MFPNumeric((axisMarks.mdYMark1 - caYAxis.mdValueFrom)/caYAxis.mdMarkInterval, true).toIntOrNanInfMFPNum();
		double dYMarkStart = axisMarks.mdYMark1 - mfpNumIntTmp.multiply(new MFPNumeric(caYAxis.mdMarkInterval, true)).doubleValue();
		...
		mfpNumIntTmp = new MFPNumeric((axisMarks.mdZMark1 - caZAxis.mdValueFrom)/caZAxis.mdMarkInterval, true).toIntOrNanInfMFPNum();
		double dZMarkStart = axisMarks.mdZMark1 - mfpNumIntTmp.multiply(new MFPNumeric(caZAxis.mdMarkInterval, true)).doubleValue();
    ...
    public CoordAxis getOppositeAxis(CoordAxis axisFrom)    {
        ...
        if (nNumofMarksInFrom == 0) {
            ...
            MFPNumeric mfpNumIntTmp = new MFPNumeric((axisFrom.marraydScaleMarks[0] - axisTo.mdValueFrom)/axisTo.mdMarkInterval, true).toIntOrNanInfMFPNum();
            double dMarkStart = axisFrom.marraydScaleMarks[0] - mfpNumIntTmp.multiply(new MFPNumeric(axisTo.mdMarkInterval, true)).doubleValue();
