// MFP project, AEPosNegOpt.java : Designed and developed by Tony Cui in 2021
...
public class AEPosNegOpt extends AbstractExpr {
    ...
    public boolean isEqual(AbstractExpr aexpr, ProgContext progContext) throws JFCALCExpErrException {
        if (menumAEType != aexpr.menumAEType)    {
            ...
            for (int idx = 0; idx < mlistChildren.size(); idx ++)    {
                if (mlistChildren.get(idx).isEqual(((AEPosNegOpt)aexpr).mlistChildren.get(idx), progContext) == false)    {
                    return false;
                }
            }
            return true;
    ...
    public int getHashCode()  throws JFCALCExpErrException {
        ...
        for (int idx = 0; idx < mlistOpts.size(); idx ++)    {
            hashRet = hashRet * 13 + mlistOpts.get(idx).hashCode();
        }
        for (int idx = 0; idx < mlistChildren.size(); idx ++) {
            hashRet = hashRet * 19 + mlistChildren.get(idx).getHashCode();
        }
        return hashRet;        
    }
    ...
    public boolean isPatternMatch(AbstractExpr aePattern,
        ...
        for (int idx = 0; idx < mlistOpts.size(); idx ++)   {
            ...
            if (bExprAdd == bPatternAdd) {  // both are + or -
                ...
                AEConst aeMinus1 = new AEConst(new DataClassSingleNum(DATATYPES.DATUM_MFPINT, MFPNumeric.MINUS_ONE));
    ...
    public DataClass evaluateAExprQuick(LinkedList<UnknownVariable> lUnknownVars, ProgContext progContext)
        ...
        DataClass datumLast = new DataClassSingleNum(DATATYPES.DATUM_MFPDEC, MFPNumeric.ZERO);
        for (int idx = 0; idx < mlistChildren.size(); idx ++)    {
            DataClass datum = mlistChildren.get(idx).evaluateAExprQuick(lUnknownVars, progContext);
            if (mlistOpts.get(idx).getOperatorType() == OPERATORTYPES.OPERATOR_ADD
                    || mlistOpts.get(idx).getOperatorType() == OPERATORTYPES.OPERATOR_POSSIGN) {
                if ( idx == 0 ) {
                    datumLast = datum;
    ...
    public AbstractExpr evaluateAExpr(LinkedList<UnknownVariable> lUnknownVars, ProgContext progContext)
        ...
        for (int idx0 = 0; idx0 < mlistChildren.size(); idx0 ++)    {
            ...
            for (; idx < mlistChildren.size(); idx ++)    {
                AbstractExpr aexpr = mlistChildren.get(idx).evaluateAExpr(lUnknownVars, progContext);
                if (aexpr instanceof AEConst) {
                    if (mlistOpts.get(idx).getOperatorType() == OPERATORTYPES.OPERATOR_ADD
                            || mlistOpts.get(idx).getOperatorType() == OPERATORTYPES.OPERATOR_POSSIGN) {
                        if ( idx == idx0) {
                            datumLast = ((AEConst)aexpr).getDataClassRef();
                        } else {
    ...
    public AbstractExpr simplifyAExpr(LinkedList<UnknownVariable> lUnknownVars, SimplifyParams simplifyParams, ProgContext progContext)
        ...
        for (int idx = 0; idx < aeCopy.mlistChildren.size(); idx ++)    {
            AbstractExpr aexpr = aeCopy.mlistChildren.get(idx).simplifyAExpr(lUnknownVars, simplifyParams, progContext);
        ...
        for (int idx = 0; idx < aeCopy.mlistChildren.size(); idx ++)    {
            ...
            if (aeCurrent instanceof AEConst
                    && DCHelper.isDataClassType(((AEConst)aeCurrent).getDataClassRef(), DATATYPES.DATUM_STRING))    {
                bB4String = false;
            }
        ...
        for (int idx = 0; idx < listChildren.size() - 1; idx ++)    {
            ...
            for (int idx1 = idx + 1; idx1 < listChildren.size(); idx1 ++)    {
                ...
                try    {
                    ...
                    AbstractExpr aexpr = mergeAddSub(listChildren.get(idx), listChildren.get(idx1), bIsAdd, simplifyParams.mbIgnoreMatrixDim, progContext);
        ...
        if (listChildren.size() == 1
            ...
            // expression is +value
            return listChildren.get(0).distributeAExpr(simplifyParams, progContext);
            ...
            return aexprReturn.distributeAExpr(simplifyParams, progContext);
        } else    {
    ...
    public boolean isNegligible(ProgContext progContext) throws JSmartMathErrException, JFCALCExpErrException    {
        validateAbstractExpr();
        for (int idx = 0; idx < mlistChildren.size(); idx ++)    {
            if (mlistChildren.get(idx).isNegligible(progContext) == false)    {
                return false;
            }
        }
        return true;
    ...
    public String outputWithFlag(int flag, ProgContext progContextNow)    throws JFCALCExpErrException, JSmartMathErrException {
        ...
        if (mlistChildren.size() == 1 && (mlistOpts.get(0).getOperatorType() == OPERATORTYPES.OPERATOR_ADD
                || mlistOpts.get(0).getOperatorType() == OPERATORTYPES.OPERATOR_POSSIGN))    {
            // expression is +value
            strOutput = mlistChildren.get(0).outputWithFlag(flag, progContextNow);
        } else    {
            for (int idx = 0; idx < mlistOpts.size(); idx ++)    {
                boolean bLeftHasOpt = false, bRightHasOpt = false;
                if (mlistOpts.get(idx).getOperatorType() == OPERATORTYPES.OPERATOR_ADD
    ...
    private static AbstractExpr mergeZeroAddSubOther(AbstractExpr aeInput1, AbstractExpr aeInput2, boolean bIsAdd,
        ...
        if (aeInput1 instanceof AEConst
                && DCHelper.isZeros(((AEConst)aeInput1).getDataClassRef(), false))    {
            if (aeInput1.menumAEType == ABSTRACTEXPRTYPES.ABSTRACTEXPR_VALUE)    {
                ...
                if (bIsAdd)    {
                    ...
                    && DCHelper.isZeros(((AEConst)aeInput2).getDataClassRef(), false)
    ...
    private static AbstractExpr mergeMulDivAddSubOther(AbstractExpr aeInput1, AbstractExpr aeInput2, boolean bIsAdd,
            boolean bIgnoreMatrixDim, ProgContext progContext) throws JFCALCExpErrException, JSmartMathErrException    {
        ...
        if (((AEMulDivOpt)aexprMultiDiv).mlistChildren.size() == 2)    {
            ...
            if (((AEMulDivOpt)aexprMultiDiv).mlistChildren.get(0).menumAEType == ABSTRACTEXPRTYPES.ABSTRACTEXPR_VALUE
                    && ((AEMulDivOpt)aexprMultiDiv).mlistChildren.get(1).isEqual(aexprNotMultiDiv, progContext))    {
                ...
                if (bHasEye)    {    // has a corresponding eye matrix.
                    DataClass datumEye = new DataClassNull();
                    if (narrayConstDim.length == 0)    {    // if it is not a matrix.
                        datumEye = new DataClassSingleNum(DATATYPES.DATUM_MFPINT, MFPNumeric.ONE);
                    } else    {
                    ...
                    case OPERATOR_MULTIPLY:
                        ...
                        if (((AEMulDivOpt)aexprMultiDiv).mlistOpts.get(0).getOperatorType() == OPERATORTYPES.OPERATOR_DIVIDE)   {
                            datumConst = ExprEvaluator.evaluateTwoOperandCell(new DataClassSingleNum(DATATYPES.DATUM_MFPINT, MFPNumeric.ONE), new CalculateOperator(OPERATORTYPES.OPERATOR_DIVIDE, 2), datumConst);
                        ...
                        return aexprReturn;
                    case OPERATOR_DIVIDE: // const / a, we cannot merge it with a
                    default:
                        throw new JSmartMathErrException(ERRORTYPES.ERROR_CANNOT_MERGE_TWO_ABSTRACTEXPRS);
                    }
                }
            } else if (((AEMulDivOpt)aexprMultiDiv).mlistChildren.get(1).menumAEType == ABSTRACTEXPRTYPES.ABSTRACTEXPR_VALUE
                    && ((AEMulDivOpt)aexprMultiDiv).mlistChildren.get(0).isEqual(aexprNotMultiDiv, progContext)
                ...
                if (bHasEye)    {    // has a corresponding eye matrix.
                    ...
                    case OPERATOR_DIVIDE:
                        // a / const
                        datum = ExprEvaluator.evaluateTwoOperandCell(new DataClassSingleNum(DATATYPES.DATUM_MFPINT, MFPNumeric.ONE), new CalculateOperator(OPERATORTYPES.OPERATOR_DIVIDE, 2), datumConst);
                                //BuiltinProcedures.evaluateReciprocal(datumConst); // cannot use this because it does not handle aexpr datum
                        if (bIsAdd)    {
                            calcOpt = new CalculateOperator(OPERATORTYPES.OPERATOR_ADD, 2);
                        } else    {
    ...
    private static AbstractExpr mergeLeftDivAddSubOther(AbstractExpr aeInput1, AbstractExpr aeInput2, boolean bIsAdd, boolean bIgnoreMatrixDim, ProgContext progContext) throws JFCALCExpErrException, JSmartMathErrException    {
        ...
        if (((AELeftDivOpt)aexprLeftDiv).maeLeft.menumAEType == ABSTRACTEXPRTYPES.ABSTRACTEXPR_VALUE
                && ((AELeftDivOpt)aexprLeftDiv).maeRight.isEqual(aexprNotLeftDiv, progContext))    {
            ...
            if (bHasEye)    {    // has a corresponding eye matrix.
                ...
                AEConst aeConst;
                AbstractExpr aexprReturn = AEInvalid.AEINVALID;
                datum = ExprEvaluator.evaluateTwoOperandCell(datumConst, new CalculateOperator(OPERATORTYPES.OPERATOR_LEFTDIVIDE, 2),
    ...
    private static AbstractExpr mergeLeftDivAddSubLeftDiv(AbstractExpr aeInput1, AbstractExpr aeInput2, boolean bIsAdd, boolean bIgnoreMatrixDim, ProgContext progContext) throws JFCALCExpErrException, JSmartMathErrException    {
        if (((AELeftDivOpt)aeInput1).maeLeft.menumAEType == ABSTRACTEXPRTYPES.ABSTRACTEXPR_VALUE
                ...
                && ((AELeftDivOpt)aeInput1).maeRight.isEqual(((AELeftDivOpt)aeInput2).maeRight, progContext))    {
            ...
            DataClass datum1 =  ExprEvaluator.evaluateTwoOperandCell(aeConstChild1.getDataClassRef(),
            ...
            DataClass datum2 = ExprEvaluator.evaluateTwoOperandCell(aeConstChild2.getDataClassRef(),
                    new CalculateOperator(OPERATORTYPES.OPERATOR_LEFTDIVIDE, 2),
                    new DataClassSingleNum(DATATYPES.DATUM_MFPINT, MFPNumeric.ONE));
            ...
            return aexprReturn;
        } else if (((AELeftDivOpt)aeInput1).maeLeft.isEqual(((AELeftDivOpt)aeInput2).maeLeft, progContext))    {
            // same a \ * and same a \ *
            ...
            AbstractExpr aexpr = mergeAddSub(aeChild1, aeChild2, bIsAdd, bIgnoreMatrixDim, progContext);
    ...
    private static AbstractExpr mergeMulDivAddSubMulDiv(AbstractExpr aeInput1, AbstractExpr aeInput2, boolean bIsAdd, ProgContext progContext) throws JFCALCExpErrException, JSmartMathErrException    {
        if (aeInput1.isEqual(aeInput2, progContext))    {
            // aeInput1 and aeInput2 are the same
            if (bIsAdd == false)    {
                // aeInput1 - aeInput2, return 0
                DataClass datumReturn = new DataClassSingleNum(DATATYPES.DATUM_MFPINT, MFPNumeric.ZERO);
                ...
                DataClass datumConst = new DataClassSingleNum(DATATYPES.DATUM_MFPINT, MFPNumeric.TWO);
            ...
            for (int idx = 0; idx < ((AEMulDivOpt)aeInput1).mlistChildren.size(); idx ++)    {
                if (((AEMulDivOpt)aeInput1).mlistChildren.get(idx)
                        .isEqual(((AEMulDivOpt)aeInput2).mlistChildren.get(idx), progContext) == false
            ...
            if (bIsMultiplyChild1 == bIsMultiplyChild2 && bIsMultiplyChild1)    {    // ... * const1 ... and ... * const2 ...
                ...
                DataClass datum1 = ExprEvaluator.evaluateTwoOperandCell(
                        new DataClassSingleNum(DATATYPES.DATUM_MFPINT, MFPNumeric.ONE),
                        new CalculateOperator(OPERATORTYPES.OPERATOR_DIVIDE, 2),
                        datumConst1);
                                    //BuiltinProcedures.evaluateReciprocal(datumConst1); // cannot use this because it does not handle aexpr datum
                DataClass datum2 = ExprEvaluator.evaluateTwoOperandCell(
                        new DataClassSingleNum(DATATYPES.DATUM_MFPINT, MFPNumeric.ONE),
                        new CalculateOperator(OPERATORTYPES.OPERATOR_DIVIDE, 2),
                        datumConst2);
                ...
                datum = ExprEvaluator.evaluateTwoOperandCell(
                        new DataClassSingleNum(DATATYPES.DATUM_MFPINT, MFPNumeric.ONE),
                        new CalculateOperator(OPERATORTYPES.OPERATOR_DIVIDE, 2),
                        datumConst2);
                        //BuiltinProcedures.evaluateReciprocal(datumConst); // cannot use this because it does not handle aexpr datum
            ...
            for (; (idxChildLong < aexprLong.mlistChildren.size()) && (idxChildShort < aexprShort.mlistChildren.size());
                ...
                if (aexprLong.mlistChildren.get(idxChildLong).isEqual(aexprShort.mlistChildren.get(idxChildShort), progContext) == false
            ...
            if (aexprLong.mlistOpts.get(idxUnequalLong).getOperatorType() == OPERATORTYPES.OPERATOR_DIVIDE)    {
                ...
                datumConst = ExprEvaluator.evaluateTwoOperandCell(
                        new DataClassSingleNum(DATATYPES.DATUM_MFPINT, MFPNumeric.ONE),
                        new CalculateOperator(OPERATORTYPES.OPERATOR_DIVIDE, 2),
                        datumConst);
                             //BuiltinProcedures.evaluateReciprocal(datumConst); // cannot use this because it does not handle aexpr datum
     ...
     */
    public static AbstractExpr mergeAddSub(AbstractExpr aeInput1, AbstractExpr aeInput2, boolean bIsAdd, boolean bIgnoreMatrixDim, ProgContext progContext) throws JFCALCExpErrException, JSmartMathErrException    {
        ...
        if (aeInput1 instanceof AEConst && aeInput2 instanceof AEConst)    {
        ...
        } else if (aeInput1.isEqual(aeInput2, progContext) && (!(aeInput1 instanceof AEConst) || !(aeInput2 instanceof AEConst)))    {
        ...
        } else if ((nPosNegFor1And2 = mustBeNegativeOrPositive(aeInput1, aeInput2, progContext)) != 0) {
            ...
            return mergeLeftDivAddSubLeftDiv(aeInput1, aeInput2, bIsAdd, bIgnoreMatrixDim, progContext);
        } else if((aeInput1.menumAEType == ABSTRACTEXPRTYPES.ABSTRACTEXPR_BOPT_MULTIPLYDIV)
                && (aeInput2.menumAEType == ABSTRACTEXPRTYPES.ABSTRACTEXPR_BOPT_MULTIPLYDIV))    {
            // both of aeInput1 and aeInput2 are multiply division.
            return mergeMulDivAddSubMulDiv(aeInput1, aeInput2, bIsAdd, progContext);
            ...
            if ((aeInput1.menumAEType == ABSTRACTEXPRTYPES.ABSTRACTEXPR_BOPT_MULTIPLYDIV)
                ...
                try    {
                    return mergeMulDivAddSubOther(aeInput1, aeInput2, bIsAdd, bIgnoreMatrixDim, progContext);
                } catch (JSmartMathErrException e)    {
                    if (e.m_se.m_enumErrorType == ERRORTYPES.ERROR_CANNOT_MERGE_TWO_ABSTRACTEXPRS)    {
            ...
            if ((aeInput1.menumAEType == ABSTRACTEXPRTYPES.ABSTRACTEXPR_BOPT_LEFTDIV)
                ...
                try    {
                    return mergeLeftDivAddSubOther(aeInput1, aeInput2, bIsAdd, bIgnoreMatrixDim, progContext);
                } catch (JSmartMathErrException e)    {
                    if (e.m_se.m_enumErrorType == ERRORTYPES.ERROR_CANNOT_MERGE_TWO_ABSTRACTEXPRS)    {
                        ;    // do nothing but handle the exception.
                    }
        ...
        for (int idx = 0; idx < mlistChildren.size(); idx ++) {
            if (mlistChildren.get(idx) instanceof AEConst
                ...
                try {
                    ...
                    // will not be here because ((AEConst)mlistChildren.get(idx)).getDataClassRef() is a confirmed DataClassAExpr type 
                    e.printStackTrace();
                }
            } else {
                listChildrenCvted.add(mlistChildren.get(idx));
