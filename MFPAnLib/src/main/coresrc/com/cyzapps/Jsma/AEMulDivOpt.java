// MFP project, AEMulDivOpt.java : Designed and developed by Tony Cui in 2021
...
public class AEMulDivOpt extends AbstractExpr {
            ...
            case OPERATOR_DIVIDE:
                if (narrayOprd.length == 0)    {    // right-operand is a number, not a matrix
                    // do nothing because narrayDimReturn will not change
                        ...
                        && ((AEConst)mlistChildren.get(0)).getDataClassRef().isEqual(new DataClassSingleNum(DATATYPES.DATUM_MFPDEC, MFPNumeric.ONE))) {
    ...
    public boolean isEqual(AbstractExpr aexpr, ProgContext progContext) throws JFCALCExpErrException {
        if (menumAEType != aexpr.menumAEType)    {
            ...
            for (int idx = 0; idx < mlistChildren.size(); idx ++)    {
                if (mlistChildren.get(idx).isEqual(((AEMulDivOpt)aexpr).mlistChildren.get(idx), progContext) == false)    {
                    return false;
                }
            }
            return true;
        ...
        for (int idx = 0; idx < mlistOpts.size(); idx ++)   {
            if (mlistOpts.get(idx).getOperatorType() == ((AEMulDivOpt)aePattern).mlistOpts.get(idx).getOperatorType())  {
                ...
                if (aeThisChild instanceof AEPowerOpt) {
                    ...
                    try {
                        datum = ExprEvaluator.evaluateTwoOperandCell(new DataClassSingleNum(DATATYPES.DATUM_MFPINT, MFPNumeric.ONE), new CalculateOperator(OPERATORTYPES.OPERATOR_DIVIDE, 2), datum);
    ...
    @Override
    public AbstractExpr distributeAExpr(SimplifyParams simplifyParams, ProgContext progContext) throws JFCALCExpErrException, JSmartMathErrException, InterruptedException    {
        validateAbstractExpr();
        int nNeedDistrIdx = -1;
        ...
        if (nNeedDistrIdx == -1)    {
            ...
            if (aeAddSub.mlistOpts.get(0).getOperatorType() == OPERATORTYPES.OPERATOR_ADD
                ...
                // negative sign
                aeReturn.mlistChildren.set(nNeedDistrIdx, new AEConst(new DataClassSingleNum(DATATYPES.DATUM_MFPDEC, MFPNumeric.MINUS_ONE)));
            ...
            for (int idx = 0; idx < aeAddSub.mlistChildren.size(); idx ++)    {
                ...
                aexprDistributedChild = aexprDistributedChild.distributeAExpr(simplifyParams, progContext);
    ...
    public DataClass evaluateAExprQuick(LinkedList<UnknownVariable> lUnknownVars, ProgContext progContext)
        ...
        DataClass datumLast = new DataClassSingleNum(DATATYPES.DATUM_MFPDEC, MFPNumeric.ONE);
        for (int idx = 0; idx < mlistChildren.size(); idx ++)    {
            DataClass datum = mlistChildren.get(idx).evaluateAExprQuick(lUnknownVars, progContext);
            if (idx == 0 && mlistOpts.getFirst().getOperatorType() == OPERATORTYPES.OPERATOR_MULTIPLY) {
                datumLast = datum;  // this handles the situation 1*INFi != infi
            } else {
    ...
    public AbstractExpr evaluateAExpr(LinkedList<UnknownVariable> lUnknownVars, ProgContext progContext)
        ...
        DataClass datumOne = new DataClassSingleNum(DATATYPES.DATUM_MFPDEC, MFPNumeric.ONE);
        for (int idx0 = 0; idx0 < mlistChildren.size(); idx0 ++)    {
            ...
            for (; idx < mlistChildren.size(); idx ++)    {
                ...
                if (aexpr instanceof AEConst) {
                    if (idx == idx0 && mlistOpts.get(idx).getOperatorType() == OPERATORTYPES.OPERATOR_MULTIPLY) {
                        datumLast = ((AEConst)aexpr).getDataClassRef();  // this handles the situation 1*INFi != infi
                    } else {
    ...
    // in this function, we are not able to consider a special case that 1 * infi != infi
    // because we may reorder the operands.
    @Override
    public AbstractExpr simplifyAExpr(LinkedList<UnknownVariable> lUnknownVars, SimplifyParams simplifyParams, ProgContext progContext)
        ...
        for (int idx = 0; idx < aeCopy.mlistChildren.size() - 1; idx ++)    {
            ...
            try    {
                ...
                AbstractExpr aexpr = mergeMultiplyDiv(aeCopy.mlistChildren.get(idx), aeCopy.mlistChildren.get(idx + 1),
                        bIsMultiply1, bIsMultiply2, cow, simplifyParams, progContext);
        ...
        if (aeCopy.mlistChildren.size() == 1)  {
            if (aeCopy.mlistOpts.get(0).getOperatorType() == OPERATORTYPES.OPERATOR_MULTIPLY)    {
                return aeCopy.mlistChildren.get(0).distributeAExpr(simplifyParams, progContext);    // distribute may not clone itself.
            } else {    // divide
                aeCopy.mlistChildren.addFirst(new AEConst(new DataClassSingleNum(DATATYPES.DATUM_MFPDEC, MFPNumeric.ONE)));
                aeCopy.mlistOpts.addFirst(new CalculateOperator(OPERATORTYPES.OPERATOR_MULTIPLY, 2));
                AbstractExpr aexprReturn = aeCopy.distributeAExpr(simplifyParams, progContext);
                return aexprReturn;
            }
        } else    {
            AbstractExpr aexprReturn = aeCopy.distributeAExpr(simplifyParams, progContext);
    ...
    public boolean isNegligible(ProgContext progContext) throws JSmartMathErrException, JFCALCExpErrException    {
        ...
        for (int idx = 0; idx < mlistChildren.size(); idx ++)    {
            if (mlistOpts.get(idx).getOperatorType() == OPERATORTYPES.OPERATOR_MULTIPLY)    {
                if (mlistChildren.get(idx).isNegligible(progContext))    {
                    return true;
                }
            }
        }
    ...
    public String outputWithFlag(int flag, ProgContext progContextNow)    throws JFCALCExpErrException, JSmartMathErrException {
        ...
        if (mlistOpts.get(0).getOperatorType() == OPERATORTYPES.OPERATOR_MULTIPLY)  {
            ...
            if (bNeedBracketsWhenToStr) {
                strOutput = "1/(" + mlistChildren.get(0).outputWithFlag(flag, progContextNow) + ")";
            } else  {
                strOutput = "1/" + mlistChildren.get(0).outputWithFlag(flag, progContextNow);
            }
        }
    ...
    private static AbstractExpr mergeZeroMultiplyDivOther(AbstractExpr aeInput1, AbstractExpr aeInput2,
        ...
        if (aeInput1 instanceof AEConst
                && !DCHelper.isDataClassType(((AEConst)aeInput1).getDataClassRef(), DATATYPES.DATUM_REF_DATA)
                && DCHelper.isZeros(((AEConst)aeInput1).getDataClassRef(), false)
                && bIsMultiply1 == true)    {
            return aeInput1;
        } else if (aeInput2 instanceof AEConst
                && !DCHelper.isDataClassType(((AEConst)aeInput2).getDataClassRef(), DATATYPES.DATUM_REF_DATA)
    ...
    private static AbstractExpr mergeEyeMultiplyDivOther(AbstractExpr aeInput1, AbstractExpr aeInput2,
        ...
        if (aeInput1 instanceof AEConst && DCHelper.isEye(((AEConst)aeInput1).getDataClassRef(), false))    {
            if (aeInput1.menumAEType == ABSTRACTEXPRTYPES.ABSTRACTEXPR_VALUE)    {
                ...
                return aeInput2;
            } else if (!(aeInput2 instanceof AEConst && DCHelper.isEye(((AEConst)aeInput2).getDataClassRef(), false)
    ...
    private static AbstractExpr mergePowerMultiplyDivOther(AbstractExpr aeInput1, AbstractExpr aeInput2,
            ...
            SimplifyParams simplifyParams, ProgContext progContext) throws JFCALCExpErrException, JSmartMathErrException, InterruptedException    {
        boolean bIsMultiply = (bIsMultiply1 == bIsMultiply2);
        ...
        if (((AEPowerOpt)aexprPower).maeLeft.isEqual(aexprNotPower, progContext)
            ...
            if (bIsMultiply)    {
                ...
                listPowerToChildren.add(new AEConst(new DataClassSingleNum(DATATYPES.DATUM_MFPINT, MFPNumeric.ONE)));
            ...
            AbstractExpr aePowerTo = new AEPosNegOpt(listPowerToChildren, listPowerToOpts)
                    .simplifyAExprMost(new LinkedList<UnknownVariable>(), simplifyParams, progContext);
    ...
    private static AbstractExpr mergePowerMultiplyDivPower(AbstractExpr aeInput1, AbstractExpr aeInput2,
            ...
            SimplifyParams simplifyParams, ProgContext progContext) throws JFCALCExpErrException, JSmartMathErrException, InterruptedException    {
        boolean bIsMultiply = (bIsMultiply1 == bIsMultiply2);
        if (((AEPowerOpt)aeInput1).maeLeft.isEqual(((AEPowerOpt)aeInput2).maeLeft, progContext)
        ...
        } else if (((AEPowerOpt)aeInput1).maeRight.isEqual(((AEPowerOpt)aeInput2).maeRight, progContext)
    ...
    private static AbstractExpr mergeConstMultiplyDivConst(AbstractExpr aeInput1, AbstractExpr aeInput2,
        ...
        if (aeInput1 instanceof AEConst && aeInput2 instanceof AEConst)    {
            ...
            if (bIsMultiply1 && bIsMultiply2)   {
                ...
                calcOpt = new CalculateOperator(OPERATORTYPES.OPERATOR_DIVIDE, 2);
                datum = ExprEvaluator.evaluateTwoOperandCell(new DataClassSingleNum(DATATYPES.DATUM_MFPDEC, MFPNumeric.ONE), calcOpt, datum1);
                calcOpt = new CalculateOperator(OPERATORTYPES.OPERATOR_MULTIPLY, 2);
    ...
    private static AbstractExpr mergeMultiplyDivSame(AbstractExpr aeInput1, AbstractExpr aeInput2,
            boolean bIsMultiply1, boolean bIsMultiply2, CalcOptrWrap cowMerged, ProgContext progContext) throws JFCALCExpErrException, JSmartMathErrException    {
        if (aeInput1.isEqual(aeInput2, progContext))     {
            if (bIsMultiply1 == bIsMultiply2)    {
                DataClass datumPower = new DataClassSingleNum(DATATYPES.DATUM_MFPINT, MFPNumeric.TWO);
    ...
    private static AbstractExpr mergeMinus1MultiplyDivPosNeg(AbstractExpr aeInput1, AbstractExpr aeInput2,
        ...
        if (aeInput1 instanceof AEPosNegOpt && aeInput2 instanceof AEConst
                && ((AEConst)aeInput2).getDataClassRef().isEqual(new DataClassSingleNum(DATATYPES.DATUM_MFPDEC, MFPNumeric.MINUS_ONE)))   {
            ...
            return aeReturn;
                ...
                && ((AEConst)aeInput1).getDataClassRef().isEqual(new DataClassSingleNum(DATATYPES.DATUM_MFPDEC, MFPNumeric.MINUS_ONE)))  {
    ...
    public static AbstractExpr mergeMultiplyDiv(AbstractExpr aeInput1, AbstractExpr aeInput2,
        ...
        if (aeInput1 instanceof AEConst && aeInput2 instanceof AEConst)    {
        ...
        } else if (aeInput1.isEqual(aeInput2, progContext))    {
            // aeInput1 equals aeInput2
            return mergeMultiplyDivSame(aeInput1, aeInput2, bIsMultiply1, bIsMultiply2, cowMerged, progContext);
            ...
            return mergePowerMultiplyDivPower(aeInput1, aeInput2, bIsMultiply1, bIsMultiply2, cowMerged, simplifyParams, progContext);
            ...
            if ((aeInput1.menumAEType == ABSTRACTEXPRTYPES.ABSTRACTEXPR_BOPT_POWER)
                ...
                try    {
                    return mergePowerMultiplyDivOther(aeInput1, aeInput2, bIsMultiply1, bIsMultiply2, cowMerged, simplifyParams, progContext);
                } catch (JSmartMathErrException e)    {
                    if (e.m_se.m_enumErrorType == ERRORTYPES.ERROR_CANNOT_MERGE_TWO_ABSTRACTEXPRS)    {
            ...
            if ((aeInput1 instanceof AEConst
                    && DCHelper.isEye(((AEConst)aeInput1).getDataClassRef(), false))
                || (aeInput2 instanceof AEConst
                        && DCHelper.isEye(((AEConst)aeInput2).getDataClassRef(), false))){
    ...
    private static AbstractExpr convertSingleChild(AbstractExpr aeChild, CalcOptrWrap cow) throws JSmartMathErrException, JFCALCExpErrException {
        if (cow.mCalcOptr.getOperatorType() == OPERATORTYPES.OPERATOR_DIVIDE && aeChild instanceof AEMulDivOpt) {
            ...
            return aeReturn;
                ...
                && ((AEConst)((AEPowerOpt)aeChild).maeRight).getDataClassRef().isEqual(new DataClassSingleNum(DATATYPES.DATUM_MFPDEC, MFPNumeric.MINUS_ONE))) {
    ...
    public AbstractExpr convertAExprDatum2AExpr() throws JSmartMathErrException {
        ...
        for (int idx = 0; idx < mlistChildren.size(); idx ++) {
            if (mlistChildren.get(idx) instanceof AEConst
                ...
                // ((AEConst)mlistChildren.get(idx)).getDataClassRef() is a DataClassBuiltIn
                try {
                    listChildrenCvted.add(DCHelper.lightCvtOrRetDCAExpr(((AEConst)mlistChildren.get(idx)).getDataClassRef()).getAExpr());
                } catch (JFCALCExpErrException e) {
                    // will not be here because the type has been checked.
                    e.printStackTrace();
                }
            } else {
                listChildrenCvted.add(mlistChildren.get(idx));
