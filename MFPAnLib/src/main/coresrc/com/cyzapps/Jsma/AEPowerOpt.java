// MFP project, AEPowerOpt.java : Designed and developed by Tony Cui in 2021
...
public class AEPowerOpt extends AbstractExpr {
    ...
    public boolean isEqual(AbstractExpr aexpr, ProgContext progContext) throws JFCALCExpErrException {
        if (menumAEType != aexpr.menumAEType)    {
            return false;
        } else if (maeLeft.isEqual(((AEPowerOpt)aexpr).maeLeft, progContext) == false)    {
            return false;
        } else if (maeRight.isEqual(((AEPowerOpt)aexpr).maeRight, progContext) == false)    {
            return false;
        } else    {
            return true;
        }
    ...
    public AbstractExpr distributeAExpr(SimplifyParams simplifyParams, ProgContext progContext) throws JFCALCExpErrException, JSmartMathErrException, InterruptedException    {
        ...
        if (maeRight instanceof AEPosNegOpt && ((AEPosNegOpt)maeRight).mlistChildren.size() > 1)    {
            ...
            return aeReturn;    // needs sort&merge after this function returns.            
                ...
                && !DCHelper.isDataClassType(((AEConst)maeRight).getDataClassRef(), DATATYPES.DATUM_REF_DATA))    {
            ...
            if (aeMulDivBase.mlistChildren.getFirst() instanceof AEConst
                    && !DCHelper.isDataClassType(((AEConst)aeMulDivBase.mlistChildren.getFirst()).getDataClassRef(), DATATYPES.DATUM_REF_DATA))    {
            ...
            if (aeMulDivBase.mlistChildren.size() > 1 && aeMulDivBase.mlistChildren.getLast() instanceof AEConst
                    && !DCHelper.isDataClassType(((AEConst)aeMulDivBase.mlistChildren.getLast()).getDataClassRef(), DATATYPES.DATUM_REF_DATA))    {
            ...
            AEConst aeOne = new AEConst(new DataClassSingleNum(DATATYPES.DATUM_MFPDEC, MFPNumeric.ONE));
            if (bLeftMostSimpleConst == false && bRightMostSimpleConst == false)    {
                ...
                if (bLeftMostSimpleConst && bRightMostSimpleConst == false)    {
                    ...
                    aeConstPart = AEMulDivOpt.mergeMultiplyDiv(aeNewLeftMost, aeNewRightMost,
                            ...
                            coLast.getOperatorType() == OPERATORTYPES.OPERATOR_MULTIPLY, cow,
                            simplifyParams, progContext);
                    co = cow.mCalcOptr;
                }
            ...
            if (((AEPosNegOpt)maeLeft).mlistOpts.get(0).getOperatorType() == OPERATORTYPES.OPERATOR_ADD
                ...
                AEConst aeMinusOne = new AEConst(new DataClassSingleNum(DATATYPES.DATUM_MFPDEC, MFPNumeric.MINUS_ONE));
    ...
    public boolean isNegligible(ProgContext progContext) throws JSmartMathErrException, JFCALCExpErrException    {
        validateAbstractExpr();
        return maeLeft.isNegligible(progContext);
    }
    ...
    public static AbstractExpr mergePower(AbstractExpr aeBase, AbstractExpr aePowerTo, boolean bIgnoreMatrixDim) throws JFCALCExpErrException, JSmartMathErrException    {
        if (aePowerTo instanceof AEConst && ((AEConst)aePowerTo).getDataClassRef().isEqual(new DataClassSingleNum(DATATYPES.DATUM_MFPDEC, MFPNumeric.ONE)))    {
            // power to 1. At this moment, we assume power base is valid to accelerate calculation.
            return aeBase;
        } else if (aePowerTo instanceof AEConst && ((AEConst)aePowerTo).getDataClassRef().isEqual(new DataClassSingleNum(DATATYPES.DATUM_MFPDEC, MFPNumeric.ZERO))) {
            ...
            if (!bIsValidPowerBase) {
                ...
                datum = new DataClassSingleNum(DATATYPES.DATUM_MFPINT, MFPNumeric.ONE);
            } else {
