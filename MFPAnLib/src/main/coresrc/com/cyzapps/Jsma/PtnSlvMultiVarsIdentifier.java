// MFP project, PtnSlvMultiVarsIdentifier.java : Designed and developed by Tony Cui in 2021
...
public class PtnSlvMultiVarsIdentifier {
    ...
    public void setPtnSlvMultiVarsIdentifier(ABSTRACTEXPRPATTERNS enumAEPType,
        ...
        for (int idx = 0; idx < mpssArray.length; idx ++)    {
            mpsaArray[idx].maePattern = ExprAnalyzer.analyseExpression(mpssArray[idx].mstrPattern, new CurPos(), listPseudoConsts, new ProgContext());
        }
    }
     ...
     */
    public static void lookupToSolveVariables(AbstractExpr aexpr, LinkedList<AbstractExpr> listAEVars, LinkedList<AbstractExpr> listAERootVars, ProgContext progContext) throws JFCALCExpErrException    {
        ...
        for (int idx = 0; idx < listAERootVars.size(); idx ++)  {
            if (aexpr.isEqual(listAERootVars.get(idx), progContext))     {
                boolean bVarHasBeenAdded = false;
                for (int idx2 = 0; idx2 < listAEVars.size(); idx2 ++)    {
                    if (aexpr.isEqual(listAEVars.get(idx2), progContext))    {
        ...
        for (int idx1 = 0; idx1 < aexpr.getListOfChildren().size(); idx1 ++)    {
            for (int idx = 0; idx < listAERootVars.size(); idx ++)    {
                if (aexpr.getListOfChildren().get(idx1).isEqual(listAERootVars.get(idx), progContext))    {
                    boolean bVarHasBeenAdded = false;
                    for (int idx2 = 0; idx2 < listAEVars.size(); idx2 ++)    {
                        if (aexpr.isEqual(listAEVars.get(idx2), progContext))    {
                            bVarHasBeenAdded = true;    // this variable has been added
        ...
        if (aexpr.menumAEType == ABSTRACTEXPRTYPES.ABSTRACTEXPR_VARIABLE)    {
            for (int idx = 0; idx < listAEVars.size(); idx ++)    {
                if (aexpr.isEqual(listAEVars.get(idx), progContext))    {
                    return;
                }
            }
            listAEVars.add(aexpr);    // during running, aexpr may change. So have to deep copy b4 this function.
            ...
            for (int idx = 0; idx < aexpr.getListOfChildren().size(); idx ++)    {
                lookupToSolveVariables(aexpr.getListOfChildren().get(idx), listAEVars, listAERootVars, progContext);
            }
        }
    }
    
    public static void lookupToSolveVarsInExprs(AbstractExpr[] aexprs, LinkedList<AbstractExpr> listAEVars, LinkedList<AbstractExpr> listAERootVars, ProgContext progContext) throws JFCALCExpErrException, JSmartMathErrException    {
        if (listAERootVars == null)    {
            listAERootVars = new LinkedList<AbstractExpr>();
        }
        for (int idx = 0; idx < aexprs.length; idx ++)    {
            // during running, aexprs may change. So have to deep copy b4 this function.
            lookupToSolveVariables(aexprs[idx], listAEVars, listAERootVars, progContext);
        }
    }
    ...
    public static boolean isSingleExprMatchPattern(AbstractExpr aeOriginalExpr,
                                                ...
                                                ProgContext progContext)    // name spaces of original expr
        ...
        aeOrginalMinusPattern = aeOrginalMinusPattern.simplifyAExprMost(listUnknown, new SimplifyParams(false, false, false), progContext);  // when we match back, we need to convert datum back to expr.
                                ...
                                ProgContext progContext) throws JFCALCExpErrException, JSmartMathErrException, InterruptedException
    {
        ...
        lookupToSolveVarsInExprs(aearrayPatterns, listPatternVars, new LinkedList<AbstractExpr>(), progContext);
            ...
            for (int idx1 = 0; idx1 < aearrayPatterns.length; idx1 ++)    {
                ...
                for (int idx2 = 0; idx2 < mpsaArray[idx1].maePConExprs.length; idx2 ++)    {
                    AbstractExpr aePConToSolve = replaceExprFunc4PCon(mpsaArray[idx1].maePConExprs[idx2],
                            arrayaeOriginalExprs[idx], listPEUMap, progContext);
                    try {
                        aePConToSolve = aePConToSolve.simplifyAExprMost(listUnknown, new SimplifyParams(false, false, false), progContext);  // for pseudo consts, need to even simplify aexpr data otherwise may lead to infinite loop when match back.
                ...
                bIsSingleExprMatchPattern = isSingleExprMatchPattern(arrayaeOriginalExprs[idx],
                        aearrayPatterns[idx1], listLocalPConVarsAll, listPEUMap, listUnknown, progContext);
                if (bIsSingleExprMatchPattern)    {
                    ...
                    for (int idx3 = 0; idx3 < listLocalPConVars.size(); idx3 ++)    {
                        ...
                        if (DCHelper.isDataClassType(datumValuePCon, DATATYPES.DATUM_ABSTRACT_EXPR)) {
                            // datumValuePCon is an instanceof DataClassBuiltIn
                            AbstractExpr aexprData = DCHelper.lightCvtOrRetDCAExpr(datumValuePCon).getAExpr()
                                    .simplifyAExprMost(listUnknown, new SimplifyParams(false, true, false), progContext);
                            if (aexprData instanceof AEConst
                                    && !DCHelper.isDataClassType(((AEConst)aexprData).getDataClassRef(), DATATYPES.DATUM_ABSTRACT_EXPR)) {
                                datumValuePCon = ((AEConst)aexprData).getDataClassRef().copySelf();
                            } else {
                                datumValuePCon = new DataClassAExpr(aexprData);
                            }
                            listLocalPConVars.get(idx3).setValue(datumValuePCon);    // we need to write it back as datumValuePCon is a new instance.
                        }
                        ...
                        if (DCHelper.isZeros(datumValuePCon, false))    {
                            ...
                            if (DCHelper.isDataClassType(datumValuePCon, DATATYPES.DATUM_REF_DATA)) {
                                ...
                                datumValuePCon = datumValuePConArray;
                            } else {
                                datumValuePCon = datumZero;
                            }
    ...
    public DataClass[] solveOriginalExprUnit(PatternExprUnitMap peuMap, LinkedList<UnknownVariable> listPseudoConstVars, ProgContext progContext)
        ...
        for (int idx1 = 0; idx1 < marrayaeVars.length; idx1 ++)    {
            if (marrayaeVars[idx1].isEqual(aePatternUnit, progContext))    {
                // find!
                AbstractExpr aeReturn = marrayaeSolveVarExprs[idx1].simplifyAExprMost(listPseudoConstVars, new SimplifyParams(false, true, false), progContext);
                ...
                if (mbarrayRootsList[idx1] && DCHelper.isDataClassType(((AEConst)aeReturn).getDataClassRef(), DATATYPES.DATUM_REF_DATA))    {
    ...
    public LinkedList<DataClass[]> solveOriginalExprUnits(LinkedList<PatternExprUnitMap> listPEUMap,
            LinkedList<UnknownVariable> listPseudoConstVars, ProgContext progContext) throws JFCALCExpErrException, InterruptedException, JSmartMathErrException    {
        LinkedList<DataClass[]> listReturnValues = new LinkedList<DataClass[]>();
        
        for (int idx = 0; idx < listPEUMap.size(); idx ++)    {
            DataClass[] datumReturnList = solveOriginalExprUnit(listPEUMap.get(idx), listPseudoConstVars, progContext);
