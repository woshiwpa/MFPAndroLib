// MFP project, AEDataRef.java : Designed and developed by Tony Cui in 2021
...
public class AEDataRef extends AbstractExpr {
    ...
    public boolean isEqual(AbstractExpr aexpr, ProgContext progContext) throws JFCALCExpErrException {
        if (menumAEType != aexpr.menumAEType)    {
            ...
            for (int idx = 0; idx < mlistChildren.size(); idx ++)    {
                if (mlistChildren.get(idx).isEqual(((AEDataRef)aexpr).mlistChildren.get(idx), progContext) == false)    {
                    return false;
                }
            }
            return true;
    ...
    public DataClass evaluateAExprQuick(LinkedList<UnknownVariable> lUnknownVars, ProgContext progContext)
        ...
        for (int idx = 0; idx < mlistChildren.size(); idx ++)    {
            DataClass datumChild = mlistChildren.get(idx).evaluateAExprQuick(lUnknownVars, progContext);
            datumList[idx] = datumChild;    // no need to deep copy
        }
        DataClass datumReturn = new DataClassArray(datumList);
        return datumReturn;
    }
    ...
    public AbstractExpr evaluateAExpr(LinkedList<UnknownVariable> lUnknownVars, ProgContext progContext)
        ...
        for (int idx = 0; idx < mlistChildren.size(); idx ++)    {
            AbstractExpr aeChild = mlistChildren.get(idx).evaluateAExpr(lUnknownVars, progContext);
    ...
    public boolean isNegligible(ProgContext progContext) throws JSmartMathErrException, JFCALCExpErrException    {
        ...
        for (int idx = 0; idx < mlistChildren.size(); idx ++)    {
            bIsNegligible &= mlistChildren.get(idx).isNegligible(progContext);
        }
