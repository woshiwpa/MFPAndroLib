/*
 * MFP project, TwoDExprDataCache.java : Designed and developed by Tony Cui in 2021
...
public class TwoDExprDataCache {
    ...
    public CacheNode calcFuncValue(MFPNumeric mfpVarValue, int nStepLog2, AbstractExpr aexpr, Variable var, ProgContext progContext)
    {
        if (mfpVarValue.isActuallyZero()) {
            ...
            if (mlistPositives.size() > 0 && mlistPositives.getFirst().mbFuncValueGot) {
                ...
                try {
                    ...
                    //var.setValue(new DataClass(DATATYPES.DATUM_MFPDEC, MFPNumeric.ZERO));
                    mlistPositives.getFirst().mdatumFuncValue = aexpr.evaluateAExprQuick(new LinkedList<UnknownVariable>(), progContext);
            ...
            if (nValueLog2 >= 0) {    // 1 or larger
                ...
                if (nNodeIdx < 0 || nNodeIdx >= listNodes.size()) {
                    ...
                    try {
                        cNodeReturn.mdatumFuncValue = aexpr.evaluateAExprQuick(new LinkedList<UnknownVariable>(), progContext);
                ...
                if (!listNodes.get(nNodeIdx).mbFuncValueGot) {
                    try {
                        var.setValue(new DataClassSingleNum(DATATYPES.DATUM_MFPDEC, listNodes.get(nNodeIdx).mmfpVarValue));
                        listNodes.get(nNodeIdx).mdatumFuncValue = aexpr.evaluateAExprQuick(new LinkedList<UnknownVariable>(), progContext);
