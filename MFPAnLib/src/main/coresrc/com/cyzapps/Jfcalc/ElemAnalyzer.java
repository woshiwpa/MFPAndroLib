// MFP project, ElemAnalyzer.java : Designed and developed by Tony Cui in 2021
...
public class ElemAnalyzer    {
                                      ...
                                      int nLastPushedCellType) throws JFCALCExpErrException
    {
        ...
        case '\\': /* If it is '\'. */
            ...
            break;
        case '&':  /* If it is AND. */
            COPTRReturnValue = new CalculateOperator(OPERATORTYPES.OPERATOR_BITWISEAND, 2);
            break;
        case '|':  /* If it is OR. */
            COPTRReturnValue = new CalculateOperator(OPERATORTYPES.OPERATOR_BITWISEOR, 2);
            break;
        case '^':  /* If it is XOR. */
            COPTRReturnValue = new CalculateOperator(OPERATORTYPES.OPERATOR_BITWISEXOR, 2);
            break;
        case '~':  /* If it is NOT. */
                                ...
                                CalculateOperator COPTR2nd)
    {
        ...
        else if (COPTR2nd.getOperatorType().getValue() > COPTR1st.getOperatorType().getValue())
        {
            if ((COPTR2nd.getOperatorType().getValue() <= OPERATORTYPES.OPERATOR_NOSMALLER.getValue()
               ...
               (COPTR2nd.getOperatorType().getValue() <= OPERATORTYPES.OPERATOR_BITWISEXOR.getValue()
                && COPTR1st.getOperatorType().getValue() >= OPERATORTYPES.OPERATOR_BITWISEAND.getValue()))
                return false;
            return true;
        }
    ...
    public static boolean isStartNumberChar(String strExpression, int nCurPos)
    {
        ...
        if ((cCurChar >= '0' && cCurChar <= '9')
                || (cCurChar == '.' && strExpression.length() >= (nCurPos + 1)  // . can be start of a number of a member of a class.
    ...
    public static DataClass getNumber(String strExpression, CurPos curpos) throws JFCALCExpErrException
    {
        ...
        if (nPN == 2 || nPN == 8 || nPN == 16)   {  // these does not support scientific notation like 0.11e11
            ...
            if (boolIsComplexImage)    {
                ...
                datumReturnNum = new DataClassComplex(MFPNumeric.ZERO, mfpNumCurNum);
            }
        } else  {   // nPN == 10
            MFPNumeric mfpNumCurNum = MFPNumeric.ZERO;
    ...
    public static DataClass getDataRef(String strExpression, CurPos curpos, ProgContext progContext) throws JFCALCExpErrException, InterruptedException
    {
        ...
        DataClass datumReturnNum = new DataClassArray(dataList);
    ...
    /**
     * 
     * this function returns a shrinked raw name (without CS, with partial CS or with full CS) OF A FUNCTION.
     * However, the string may not be capitalized. Moreover, it doesn't support * or zero length namespace.
    */
    public static String getShrinkedFuncNameStr(String strExpression, CurPos curpos, String strRecognizedPart) throws JFCALCExpErrException {
        while(curpos.m_nPos < strExpression.length() && isBlankChar(strExpression, curpos.m_nPos)) {
            curpos.m_nPos ++; // ignore all the blanks.
        }
        String strShrinkedName = "";
        if (strExpression.length() <= curpos.m_nPos) {
            strShrinkedName = strRecognizedPart;
        } else if (curpos.m_nPos <= strExpression.length() - 2
                && strExpression.charAt(curpos.m_nPos) == ':'
                && strExpression.charAt(curpos.m_nPos + 1) == ':')    {
            if (strRecognizedPart.length() == 0 || isNameChar(strRecognizedPart, strRecognizedPart.length() - 1) != 0) {
                strShrinkedName += strRecognizedPart + "::";
                curpos.m_nPos += 2;
                while(curpos.m_nPos < strExpression.length() && isBlankChar(strExpression, curpos.m_nPos)) {
                    curpos.m_nPos ++;
                }
                strShrinkedName = getShrinkedFuncNameStr(strExpression, curpos, strShrinkedName);
            } else {
                throw new JFCALCExpErrException(ERRORTYPES.ERROR_INVALID_NAME);
            }
        } else if (isNameChar(strExpression, curpos.m_nPos) == 1) {
            // is the start of the name
            if (strRecognizedPart.length() == 0
                    || (strRecognizedPart.length() >= 2
                        && (strRecognizedPart.charAt(strRecognizedPart.length() - 2) == ':'
                                && strRecognizedPart.charAt(strRecognizedPart.length() - 1) == ':')
                            )
                    ) {
                while ((strExpression.length() > curpos.m_nPos) && (isNameChar(strExpression, curpos.m_nPos) >= 1))
                {
                    strShrinkedName += strExpression.charAt(curpos.m_nPos);
                    curpos.m_nPos++;
                }
                strShrinkedName = strRecognizedPart + strShrinkedName;
                strShrinkedName = getShrinkedFuncNameStr(strExpression, curpos, strShrinkedName);
            } else {
                throw new JFCALCExpErrException(ERRORTYPES.ERROR_INVALID_NAME);
            }
        ...
        if (strShrinkedName.length() == 0 || isNameChar(strShrinkedName, strShrinkedName.length() - 1) == 0) {
            // this is not a valid name.
            throw new JFCALCExpErrException(ERRORTYPES.ERROR_INVALID_NAME);
        }
        return strShrinkedName;
    }
    ...
    /***************************************************************************\
     getExprName:
       ...
       Input:
       * String strExprNameOwner: the string of the owner of the exprName.
         DataClass exprNameOwner: the owner of the exprName, generally it is an instance of a class. It can be null.
         String strExpression: The expression string.
         CurPos curpos: The initial position of the name in the expression.
         DataReferringObj referringObj: return the referring object of the returned value. This referring obj is useful in assignment.
         ProgContext progContext: the program context
       Output:
         The value of the name.
    \
     * @throws JFCALCExpErrException 
     * @throws InterruptedException ***************************************************************************/
    public static DataClass getExprName(String strExprNameOwner, DataClass exprNameOwner, String strExpression, CurPos curpos, DataReferringObj referringObj, ProgContext progContext) throws JFCALCExpErrException, InterruptedException
    {
        DataClass datumReturnNum = new DataClassNull();
        ExprEvaluator exprEvaluator = new ExprEvaluator(progContext);
        ...
        if ((strExpression.length() > nCurFurtherCharPos) && (strExpression.charAt(nCurFurtherCharPos) == '(')) /* function */
        {
            ...
            datumReturnNum = FuncEvaluator.evaluateFunction(exprEvaluator, strExprNameOwner, exprNameOwner, strName, listParamRawInputs, progContext);
            ...
            // do not return datumReturnNum; here as we will check if there is a . following a function or variable.
        }
                        ...
                        isCalcOperatorChar(strExpression, nCurFurtherCharPos))))
        {
             ...
             * is an operand.
             */
            if (exprNameOwner != null) {    // this is a member variable
                DataClassClass memberVariableOwner = DCHelper.lightCvtOrRetDCClass(exprNameOwner);
                MFPClassInstance ownerInstance = memberVariableOwner.getClassInstance();
                Variable var = ownerInstance.getMemberVariable(strExprNameOwner, strName, AccessRestriction.PRIVATE, progContext);
                if (var != null) {
                    // ok, we found it
                    datumReturnNum = var.getValue();
                    referringObj.setReferringObj(var);
                } else {                        
                    throw new JFCALCExpErrException(ERRORTYPES.ERROR_MEMBER_NOT_EXIST);
                }
            } else {    // this is not a member variable
                Variable var = VariableOperator.lookUpSpaces(strName, progContext.mdynamicProgContext.mlVarNameSpaces);
                if (var != null){
                    /*
    ...
    public static DataClass getString(String strExpression, CurPos curpos) throws JFCALCExpErrException
    {
        ...
        DataClass datumString = new DataClassString(strValue);
        curpos.m_nPos = i + 1;
        return datumString;
    }
}
