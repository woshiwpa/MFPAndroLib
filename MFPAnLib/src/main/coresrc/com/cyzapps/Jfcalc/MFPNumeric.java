/*
 * MFP project, MFPNumeric.java : Designed and developed by Tony Cui in 2021
 ...
 */
public class MFPNumeric implements Serializable {
    ...
    public static final double POWER_BASE_USE_INTEGER_LONG_MAX
            = (long)Math.pow(Long.MAX_VALUE, 1.0/POWER_EXPONENT_REASONABLE_ACCU_POS_RANGE_MAX);    // so that we can use long to calculate;
    ...
    public static final MFPNumeric ONE_AND_A_HALF_PI = new MFPNumeric(MFPNumeric.PI.mbigDecimalValue.multiply(new BigDecimal("1.5")));
    ...
    protected final long mlValue;   // = 0;
    protected final double mdValue; // = 0.0;
    protected final BigInteger mbigIntegerValue;    // = BigInteger.ZERO;
    protected final BigDecimal mbigDecimalValue;    // = BigDecimal.ZERO;
    
    protected final boolean mbIsActuallyInteger;  // = false;
    protected final double mdActualValueCompZero; // = 0;
    ...
    public final Type mType;    // = Type.MFP_DOUBLE_VALUE;  // by default, it is double value.
    ...
    public MFPNumeric() {
        ...
        mType = Type.MFP_DOUBLE_VALUE;  // by default, it is double value.
    }
    ...
    public MFPNumeric(BigDecimal bigDecValue, double dValue, BigInteger bigIntegerValue, long lValue, Type typeInfo) {
        ...
        mbIsActuallyInteger = (compareToNoRelative(mbigDecimalValue, bigDecRoundValue, mdValue, bigDecRoundValue.doubleValue(), 1) == 0);
    }
    ...
    public MFPNumeric(BigDecimal bigDecValue)   {
        ...
        long lValue = mbigIntegerValue.longValue();
        int nComparelValue = mbigIntegerValue.compareTo(BigInteger.valueOf(lValue));
        if (nComparelValue > 0)    {
            lValue = Long.MAX_VALUE;   // means long overflowed.
        } else if (nComparelValue < 0)    {
            lValue = Long.MIN_VALUE;   // means long overflowed.
        }
        mlValue = lValue;
        ...
        // it is fine.
        mdActualValueCompZero = compareToNoRelative(mdValue, 0, 1);   // use double to compare with zero so that it is fast. This is the only special case we can use compareTo(double, double).
        // setScale function is every expensive, so we check mdValue, if its ulp < 0.5, this means,
        // a. Math.round(mdValue) is accurate, and b. rounding mbigDecimalValue should get the same result
        // as rounding mdValue if mdValue in the range [xxx.0 to xxx.25] and in the range [xxx.75 to xxx.0),
        // if mdValue is very close to xxx.5, it is possible that round mdValue is round up while round mbigDecimalValue
        // is round down. However, in this case, mbigDecimalValue can never be close to an integer so that
        // compareToNoRelative will returns non-zero anyway.
        BigDecimal bigDecRoundValue = (Math.ulp(mdValue) < 0.5)?
        		BigDecimal.valueOf(Math.round(mdValue)) 
        		:mbigDecimalValue.setScale(0, RoundingMode.HALF_UP);
        // use big decimal to compare with integer so that it is accurate.
        // also note that rint treat .5 differently from rounding HALF_UP, but this doesn't matter because we only
        // need to check if it is an integer. xxx.5 definitely is not an integer.
        mbIsActuallyInteger = (compareToNoRelative(mbigDecimalValue, bigDecRoundValue, mdValue, Math.rint(mdValue), 1) == 0);
    }
    
    public MFPNumeric(BigInteger bigIntValue)   {
        long lValue = bigIntValue.longValue();
        int nComparelValue = bigIntValue.compareTo(BigInteger.valueOf(lValue));
        if (nComparelValue > 0)    {
            lValue = Long.MAX_VALUE;   // means long overflowed.
        } else if (nComparelValue < 0)    {
            lValue = Long.MIN_VALUE;   // means long overflowed.
    ...
    public MFPNumeric(double dValue, boolean bCvt2Str)   {
        ...
        if (Double.isNaN(dValue)) {
            ...
            mdValue = dValue;
            if (dValue >= Long.MAX_VALUE) {
                mlValue = Long.MAX_VALUE;
                mbigDecimalValue = BigDecimal.valueOf(dValue);	// if dValue is this big, it is actually a integer
            ...
            } else if (dValue <= Long.MIN_VALUE) {
                mlValue = Long.MIN_VALUE;
                mbigDecimalValue = BigDecimal.valueOf(dValue);	// if dValue is this big, it is actually a integer
                mbigIntegerValue = mbigDecimalValue.toBigInteger(); // cannot get bigIntegerValue from mlValue because mlValue may be overflowed value, i.e. dValue < Long.MIN_VALUE
            } else {
                mlValue = (long) dValue;
                // note that even if ulp >= 1.0, BigDecimal.valueOf(dValue) may not equal new BigDecimal(dValue);
                mbigDecimalValue = (bCvt2Str)?BigDecimal.valueOf(dValue):new BigDecimal(dValue);
                mbigIntegerValue = BigInteger.valueOf(mlValue);
            }
            mType = Type.MFP_DOUBLE_VALUE;
    ...
    public MFPNumeric(long lValue)   {
        ...
        mbigDecimalValue = BigDecimal.valueOf(lValue);
        mType = Type.MFP_INTEGER_VALUE;
    ...
    // strVal must have been lower cased.
    public MFPNumeric(String strValue)  {
    	...
    	if (strValue.equals("nan"))   {
            ...
            mbIsActuallyInteger = false;
            mdActualValueCompZero = -1;
        } else if (strValue.equals("inf") || strValue.equals("+inf"))   {
            mlValue = Long.MAX_VALUE;
            ...
            long lValue = mbigDecimalValue.longValue();
            int nComparelValue = mbigDecimalValue.toBigInteger().compareTo(BigInteger.valueOf(lValue));
            if (nComparelValue > 0)    {
                lValue = Long.MAX_VALUE;   // means long overflowed.
            } else if (nComparelValue < 0)    {
                lValue = Long.MIN_VALUE;   // means long overflowed.
            ...
            if (mbigDecimalValue.scale() <= 0)   {  // can identify 99% integer cases.
                mType = Type.MFP_INTEGER_VALUE;
                // now let's check if it is an integer and compare it to 0.
                mbIsActuallyInteger = true;
                mdActualValueCompZero = (mlValue > 0)?1:(mlValue < 0)?-1:0;
            } else {
                mType = Type.MFP_DOUBLE_VALUE;
                // now let's check if this MFPNumeric is an integer and compare it with 0.
                // we cannot use double value to check because double value's accuracy is not
    ...
    public static MFPNumeric valueOf(double dValue) {
        return new MFPNumeric(dValue, true);
    }
    ...
    public int hashCode() {
        switch (mType) {
            ...
            case MFP_INTEGER_VALUE:
                return mbigIntegerValue.hashCode();
            case MFP_DOUBLE_VALUE:
                if (isActuallyInteger()) {
                    return mbigIntegerValue.hashCode();
                } else {
                    return mbigDecimalValue.hashCode();
                }
            default:    // inf and nan
                return Double.hashCode(mdValue);
        }
    }
    ...
    public MFPNumeric toBoolMFPNum() throws ArithmeticException {
        ...
        if (mType == Type.MFP_BOOLEAN_VALUE)    {
            ...
            return new MFPNumeric((mlValue != 0));
        } else if (mType == Type.MFP_DOUBLE_VALUE || isInf())  {
            return new MFPNumeric((mdValue != 0));
        } else {
    ...
    protected static double compareToNoRelative(BigDecimal bigDecA, BigDecimal bigDecB, double dA, double dB, double errorScale)  {
        // use double to compare first because it is much quick
        double dAbsoluteErr = (errorScale == 1)?THE_MAX_ABSOLUTE_ERROR_OF_MFPNUMERIC
                :((errorScale == 0)?0:THE_MAX_ABSOLUTE_ERROR_OF_MFPNUMERIC* errorScale);
        double dAbsAMinusB = Math.abs(dA - dB);
        double dUlpError = Math.ulp(dA) + Math.ulp(dB);
        if (dAbsAMinusB > dAbsoluteErr + dUlpError)  {
            // dA and dB's gap is larger than dAbsoluteErr plus error.
            return (dA > dB)?1.0:-1.0;
        } else if (dAbsAMinusB + dUlpError <= dAbsoluteErr) {
            // even considering ulp error, dA and dB's gap is still smaller than AbsoluteErr
            return 0;
        } else {
            // use big decimal to compare.
            // do not use divide to compare. just compare value - value.
            ...
            if (nStrictlyCompare == 0)    {
                ...
                BigDecimal bigDecAbsoluteErr = (errorScale == 1)?BIG_DEC_THE_MAX_ABSOLUTE_ERROR_OF_MFPNUMERIC
                    :((errorScale == 0)?BigDecimal.ZERO:BigDecimal.valueOf(dAbsoluteErr));
    ...
    protected static double compareTo(BigDecimal bigDecA, BigDecimal bigDecB, double dA, double dB, double errorScale)    {
        double dAbsoluteErr = (errorScale == 1)?THE_MAX_ABSOLUTE_ERROR_OF_MFPNUMERIC
                :((errorScale == 0)?0:THE_MAX_ABSOLUTE_ERROR_OF_MFPNUMERIC* errorScale);
        double dRelativeErr = (errorScale == 1)?THE_MAX_RELATIVE_ERROR_OF_MFPNUMERIC
                :((errorScale == 0)?0:THE_MAX_RELATIVE_ERROR_OF_MFPNUMERIC* errorScale);
        // need to consider ulp error
        double dAbsAMinusB = Math.abs(dA - dB), dAbsA = Math.abs(dA), dAbsB = Math.abs(dB);
        double dUlpError = Math.ulp(dA) + Math.ulp(dB), dUlpErrorA = Math.ulp(dA), dUlpErrorB = Math.ulp(dB);
        if (dAbsAMinusB + dUlpError <= dAbsoluteErr && dAbsA + dUlpErrorA <= dAbsoluteErr  && dAbsB + dUlpErrorB <= dAbsoluteErr)  {
            // this is to compare very small values (values close to zero). Even considering error they are still so small so
            // that it is save to say they are equal.
            return 0;
        } else if (dAbsAMinusB > dAbsoluteErr + dUlpError && dAbsA > dAbsoluteErr + dUlpErrorA  && dAbsB > dAbsoluteErr + dUlpErrorB) {
            double dDivBy = Math.max(dAbsA, dAbsB);
            if (dDivBy == 0) {
                return 0; // dA == dB == 0 and ulp(dA) and ulp(dB) are e-304 much smaller than error tolerance, we can say they equal
            } else if ((dAbsAMinusB + dUlpError)/dDivBy <= dRelativeErr)   {
                // their relative difference is so small
                return 0;
            } else if ((dAbsAMinusB - dUlpError)/dDivBy > dRelativeErr)   {
                // their relative difference is big enough to compare
                return (dA > dB)?1:-1;
            }
        }

        // have to use big decimal to compare.
        // not check if errorScale is positive or negative or 0, assume it is valid when use.
        // we cannot compare primitive values, we have to use big decimal to compare
        ...
        BigDecimal bigDecRelativeErr = (errorScale == 1)?BIG_DEC_THE_MAX_RELATIVE_ERROR_OF_MFPNUMERIC
                :((errorScale == 0)?BigDecimal.ZERO:BigDecimal.valueOf(dRelativeErr));
    ...
    public static double compareTo(MFPNumeric a, MFPNumeric b, double errorScale) {
        ...
        return compareTo(a.mbigDecimalValue, b.mbigDecimalValue, a.mdValue, b.mdValue, errorScale);
    }
    ...
    public static MFPNumeric pow(MFPNumeric a, MFPNumeric b)  {
        ...
        if (a.isActuallyInteger() && Math.abs(a.mdValue) == 1.0)  { // avoid to use compareTo(BigDecimal, BigDecimal to save CPU time.
            ...
            return MFPNumeric.ZERO;
        } else if (a.isActuallyInteger() && b.isActuallyInteger()
                && b.isActuallyPositive() && b.intValue() <= POWER_EXPONENT_REASONABLE_ACCU_POS_RANGE_MAX
                && a.intValue() <= POWER_BASE_USE_INTEGER_LONG_MAX && a.intValue() >= -POWER_BASE_USE_INTEGER_LONG_MAX) {
            // integer ** integer and returned value is still in long's range.
            long lResult = 1;
            for (int idx = 0; idx < b.intValue(); idx ++) {
                lResult *= a.intValue();
            }
            return new MFPNumeric(lResult);
        } else {
            ...
            if (dAbsBValue <= POWER_EXPONENT_REASONABLE_ACCU_POS_RANGE_MAX)   {
                if (dAbsAValue <= PRIMITIVE_DOUBLE_REASONABLE_ACCU_POS_RANGE_MAX && dAbsAValue >= PRIMITIVE_DOUBLE_REASONABLE_ACCU_POS_RANGE_MIN)   {
                    ...
                    if (dAbsResult <= PRIMITIVE_DOUBLE_REASONABLE_ACCU_POS_RANGE_MAX && dAbsResult >= PRIMITIVE_DOUBLE_REASONABLE_ACCU_POS_RANGE_MIN)   {
                        ...
                        return new MFPNumeric(dResult, false);
                    }
                }
            }
            ...
            // now bigDecCurrentB is in (-1, 1). Do not use compareTo because we are checking zero.
            if (compareToNoRelative(bigDecCurrentB, BigDecimal.ZERO, bigDecCurrentB.doubleValue(), 0.0, 1) != 0)    {   // bigDecCurrentB is not actually 0
                ...
                BigDecimal bigDecAStep = BigDecimal.valueOf(nAStep);
                BigDecimal bigDec1OverAStep = new BigDecimal(1.0/nAStep);
                BigDecimal bigDecAStepToCurrentB = new BigDecimal(dAStepToCurrentB); // do not use valueOf(double) so that more accurate
                while (true)   { // new BigDecimal(1.0/nAStep) may not be accurate.
                    if (bigDecCurrentA.compareTo(bigDecAStep) >= 0)   {
                        bigDecResult = bigDecResult.multiply(bigDecAStepToCurrentB);
                        bigDecCurrentA = divide(bigDecCurrentA, bigDecAStep);
                    } else if (bigDecCurrentA.compareTo(bigDec1OverAStep) <= 0) {
                        bigDecResult = divide(bigDecResult, bigDecAStepToCurrentB);
    ...
    public static MFPNumeric sqrt(MFPNumeric a) {
        if (a.mType == Type.MFP_NAN_VALUE || a.isActuallyNegative())    {
            ...
            return new MFPNumeric(Math.sqrt(a.mdValue), false);
        } else  {
            ...
            BigDecimal bigDecAStep = BigDecimal.valueOf(nAStep);
            BigDecimal bigDecSqrtAStep = new BigDecimal(dSqrtAStep);
            while (true)    {
                if (bigDecCurrentA.compareTo(bigDecAStep) >= 0)   {
                    bigDecResult = bigDecResult.multiply(bigDecSqrtAStep);
                    bigDecCurrentA = divide(bigDecCurrentA, bigDecAStep);
                } else if (bigDecCurrentA.compareTo(new BigDecimal(1.0/nAStep)) <= 0)   {
                    bigDecResult = divide(bigDecResult, bigDecSqrtAStep);
                    bigDecCurrentA = bigDecCurrentA.multiply(bigDecAStep);
    ...
    public static MFPNumeric hypot(MFPNumeric x, MFPNumeric y) {
        if (x.mType == Type.MFP_NAN_VALUE || y.mType == Type.MFP_NAN_VALUE)    {
            ...
            if (bigDecXYRatioSqr.compareTo(new BigDecimal(PRIMITIVE_DOUBLE_REASONABLE_ACCU_POS_RANGE_MIN)) < 0) {
                ...
                // Math.sqrt(1 + (x/y)**2)
                BigDecimal bigDecReturn = bigDecBase.multiply(BigDecimal.ONE.add(divide(bigDecXYRatioSqr, BigDecimal.valueOf(2))));
    ...
    public static MFPNumeric cbrt(MFPNumeric a) {
        if (a.mType == Type.MFP_NAN_VALUE)    {
            ...
            // use double to calculate, same accuracy, higher speed.
            return new MFPNumeric(Math.cbrt(a.mdValue), false);
        } else  {
            ...
            BigDecimal bigDecCbrtAStep = new BigDecimal(dCbrtAStep);
            while (true)    {
                if (bigDecCurrentAbsA.compareTo(bigDecAStep) >= 0)   {
                    bigDecResult = bigDecResult.multiply(bigDecCbrtAStep);
                    bigDecCurrentAbsA = divide(bigDecCurrentAbsA, bigDecAStep);
                } else if (bigDecCurrentAbsA.compareTo(bigDec1OverAStep) <= 0)   {
                    bigDecResult = divide(bigDecResult, bigDecCbrtAStep);
                    bigDecCurrentAbsA = bigDecCurrentAbsA.multiply(bigDecAStep);
    ...
    public static MFPNumeric exp(MFPNumeric a) {
        if (a.mType == Type.MFP_NAN_VALUE)    {
            ...
            return new MFPNumeric(Math.exp(a.mdValue), false);
            ...
            while (bigDecCurrentA.abs().compareTo(BigDecimal.ONE) >= 0) {
                ...
                if (a.isActuallyPositive()) {
                    bigDecResult = bigDecResult.multiply(bigDecComponent);
                    bigDecCurrentA = bigDecCurrentA.subtract(BigDecimal.valueOf(nIntExponent));
                } else {    // a is negative. a is actually zero has been covered above.
                    bigDecResult = divide(bigDecResult, bigDecComponent);
                    bigDecCurrentA = bigDecCurrentA.add(BigDecimal.valueOf(nIntExponent));
                }
            }
            
            // current A is between (-1, 1) and is not zero. No need to use compareTo because we simply check zero.
            if (compareToNoRelative(bigDecCurrentA, BigDecimal.ZERO, bigDecCurrentA.doubleValue(), 0.0, 1) != 0)    {   // bigDecCurrentA is not actually 0
    ...
    public static MFPNumeric log(MFPNumeric a)  {
        if (a.mType == Type.MFP_NAN_VALUE || a.isActuallyNegative())    {
            ...
            return new MFPNumeric(Math.log(a.mdValue), false); // do not use logp1 seems log is more accurate.
        } else {
            ...
            BigDecimal bigDecALogStep = BigDecimal.valueOf(nALogStep);
            ...
            while (true)    {
                if (bigDecCurrentA.compareTo(bigDecAStep) >= 0)   {
                    bigDecResult = bigDecResult.add(bigDecALogStep);
                    bigDecCurrentA = divide(bigDecCurrentA, bigDecAStep);
                } else if (bigDecCurrentA.compareTo(bigDecOneOverAStep) <= 0)   {
                    bigDecResult = bigDecResult.subtract(bigDecALogStep);
                    bigDecCurrentA = bigDecCurrentA.multiply(bigDecAStep);
                } else {
    ...
    public static MFPNumeric log10(MFPNumeric a)  {
        if (a.mType == Type.MFP_NAN_VALUE || a.isActuallyNegative())    {
            ...
            // a is normal positive value. using primitive calculation.
            // still use primitive types to do calculation here. Otherwise, it is too complicated.
            return new MFPNumeric(Math.log10(a.mdValue), false);
        } else {
            // have to use big decimal.
            BigDecimal bigDecResult = BigDecimal.ZERO;
            BigDecimal bigDecCurrentA = a.mbigDecimalValue;
    ...
    public static MFPNumeric sin(MFPNumeric a)  {
        if (a.isNanOrInf()) {
            ...
            if (dAbsA == MFPNumeric.TWO_PI.mdValue || dAbsA == MFPNumeric.PI.mdValue || a.isActuallyZero())    {
                ...
                // pi or 5*pi/6
                return new MFPNumeric(dASigNum * 0.5, true);
                ...
                return new MFPNumeric(Math.sin(a.mdValue), false);
            }
        } else {
            ...
            return new MFPNumeric(Math.sin(bigDecAngle.doubleValue()), false);
        }
    }
    ...
    public static MFPNumeric cos(MFPNumeric a)  {
        if (a.isNanOrInf()) {
            ...
            if (dAbsA == MFPNumeric.TWO_PI.mdValue || a.isActuallyZero())    {
                ...
                return new MFPNumeric(Math.cos(a.mdValue), false);
            }
        } else {
            // use bigDecimal, do not consider special angle like pi, pi/2 to save computing time.
            BigDecimal[] bigDecArray = a.mbigDecimalValue.divideAndRemainder(MFPNumeric.TWO_PI.mbigDecimalValue);
            BigDecimal bigDecAngle = bigDecArray[1];
            return new MFPNumeric(Math.cos(bigDecAngle.doubleValue()), false);
        }
    }
    ...
    public static MFPNumeric tan(MFPNumeric a)  {
        if (a.isNanOrInf()) {
            ...
            if (dAbsA == MFPNumeric.TWO_PI.mdValue || dAbsA == MFPNumeric.PI.mdValue || a.isActuallyZero())    {
                ...
                return new MFPNumeric(Math.tan(a.mdValue), false);
            }
        } else {
            // use bigDecimal, do not consider special angle like pi, pi/2 to save computing time.
            BigDecimal[] bigDecArray = a.mbigDecimalValue.divideAndRemainder(MFPNumeric.TWO_PI.mbigDecimalValue);
            ...
            if (compareTo(bigDecAngle, MFPNumeric.PI_OVER_TWO.mbigDecimalValue,
                    bigDecAngle.doubleValue(), MFPNumeric.PI_OVER_TWO.mdValue, 1) == 0)    {
                // tan(pi/2) = inf
                return MFPNumeric.INF;
            } else if (compareTo(bigDecAngle, MFPNumeric.ONE_AND_A_HALF_PI.mbigDecimalValue,
                    bigDecAngle.doubleValue(), MFPNumeric.ONE_AND_A_HALF_PI.mdValue, 1) == 0)    {
                // tan(pi/2 * 3) = -inf
                return MFPNumeric.NEG_INF;
            } else {
                return new MFPNumeric(Math.tan(bigDecAngle.doubleValue()), false);
            }
        }
    }
    ...
    public static MFPNumeric asin(MFPNumeric a) {
        ...
        if (dCompare1 < 0 && dCompareMinus1 > 0) {
            return new MFPNumeric(Math.asin(a.mdValue), false);
        } else if (dCompare1 == 0)  {
    ...
    public static MFPNumeric acos(MFPNumeric a) {
        double dCompare1 = compareTo(a, MFPNumeric.ONE), dCompareMinus1 = compareTo(a, MFPNumeric.MINUS_ONE);
        if (dCompare1 < 0 && dCompareMinus1 > 0) {
            return new MFPNumeric(Math.acos(a.mdValue), false);
        } else if (dCompare1 == 0)  {
            return MFPNumeric.ZERO;
        } else if (dCompareMinus1 == 0) {
    ...
    public static MFPNumeric atan(MFPNumeric a) {
        if (a.mType == Type.MFP_NAN_VALUE) {
            ...
            return new MFPNumeric(Math.atan(a.mdValue), false);    // don't worry about very big value because atan(a) in that case should be pi/2 or -pi/2
        }
    }
    ...
    public static MFPNumeric atan2(MFPNumeric y, MFPNumeric x)  {
        if (y.mType == Type.MFP_NAN_VALUE || x.mType == Type.MFP_NAN_VALUE) {
            ...
            return multiply(MFPNumeric.PI, new MFPNumeric(0.75, true));
        } else if (y.mType == Type.MFP_NEGATIVE_INF && x.mType == Type.MFP_POSITIVE_INF)    {
            return multiply(MFPNumeric.PI, new MFPNumeric(-0.25, true));
            ...
            if (dYOverX < PRIMITIVE_DOUBLE_REASONABLE_ACCU_POS_RANGE_MIN && dYOverX > -PRIMITIVE_DOUBLE_REASONABLE_ACCU_POS_RANGE_MIN) {
                ...
                mfpNumAtanResult = new MFPNumeric(Math.atan(dYOverX), false);    // don't worry about very big value because atan(y/x) in that case should be pi/2 or -pi/2
            }
            if (x.isPositive()) {
    ...
    public static MFPNumeric toRadians(MFPNumeric a)   {
        if (a.mType == Type.MFP_NAN_VALUE || a.isInf()) {
            ...
            BigDecimal bigDecResult = divide(a.mbigDecimalValue.multiply(MFPNumeric.PI.mbigDecimalValue), BigDecimal.valueOf(180));
            return new MFPNumeric(bigDecResult);
        }
    }
    ...
    public static MFPNumeric toDegrees(MFPNumeric a)   {
        if (a.mType == Type.MFP_NAN_VALUE || a.isInf()) {
            ...
            // always use big decimal otherwise rounding error will be too large
            // to think that double value 1.00000000000000000000001 is actually 1.
            BigDecimal bigDecResult = divide(a.mbigDecimalValue.multiply(BigDecimal.valueOf(180)), MFPNumeric.PI.mbigDecimalValue);
            return new MFPNumeric(bigDecResult);
        }
    }
